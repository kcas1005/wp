...
<%
	...
	if(session.getAttribute("id")==null){
		...
		if(!md.isMem(id)){
			...
			//결제할 상품 관련 설정
			//직접구매인 경우
			if(request.getParameter("product_check") != null && request.getParameter("product_no") != null){
				int pno = Integer.valueOf(request.getParameter("product_no")),
					pchk = Integer.valueOf(request.getParameter("product_check"));
				
				CartVo cv = new CartVo();
				cv.setProduct_no(pno);
				cv.setPurchase(pchk);
				cartArr.add(cv);
			}
			//장바구니 구매인 경우
	...
	<h2 id="title">배송 정보</h2>
		...
		<table id="table2">
			...
			<tr>
				<td><input type="checkbox" checked name="chkbox"
					...
					%>
				></td>
				<!-- "<a href='content.jsp?reviewpage=1&reviewcategory=0&product_no="+info_product.get(i).getProduct_no()+"'>" -->
				<td>
				<% 
				out.print("<a target='_blank' "+
							"href='content.jsp?reviewpage=1&reviewcategory=0&product_no="+no+"'>");
				out.print(pd.selectNameByNo(no));
				out.print("</a>");
				%>
		...
		<div id="priceResult">
			<p>
				...
				<!-- 보유 포인트 --><input type="hidden" id="upoint" value=<%=mem.getPoint() %>>
				<!-- 직접구매 여부--><%
				if(request.getParameter("product_check") != null && request.getParameter("product_no") != null)
					out.print("<input type='hidden' name='direct' value='1'>");
				%>			
			</p>				
		</div>
