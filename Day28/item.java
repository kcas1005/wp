...
public class item {
    static int Data;
    static String[] Item_List = { "고양이의 검", "개의 검", "코끼리의 검", "고릴라의 검" };
    ...
    public static void MonsterItem() {
        ...
        for (int i = 0; i < Item_Monster.length; i++) {
            ...
            if (Item_Monster[i] != null) {
                continue;
            } else if (Item_Monster[i] == null) {
                Item_Monster[i] = Item_List[MIData];
                // Item_Monster[i]=get(Item_List[MIData]);

                System.out.println(i + 1 + "번째 창에 " + Item_List[MIData] + " 아이템을 집어넣습니다.");
