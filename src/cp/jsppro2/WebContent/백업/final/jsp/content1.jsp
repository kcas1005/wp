      ...
      <!-- 첫줄 -->
         ...
         %>    
            ...
            <p class='text_hidden'><%=articlepage.get(i).getContent()%>
               <br><b style='color:pink;'>이 리뷰가 도움이 되셨나요?</b>
               <a href='review.jsp?article_no=<%=articlepage.get(i).getArticle_no()%>' target='test<%=i %>'  style='color:pink;'>좋아요</a> 
               <%-- <b class='heart' ><%=articlepage.get(i).getRead_cnt()%> 개</b> --%>
               <iframe src='' name='test<%=i %>' scrolling = 'no'style='border:none; width:80px; height:30px; ' class='heart'>
               </iframe> 
            </p>
